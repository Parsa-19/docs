...
fetch and install by script: <br>
`curl -fsSL https://raw.githubusercontent.com/v2fly/fhs-install-v2ray/master/install-release.sh | sudo bash` <br>

verify: <br>
`v2ray version` <br>

then the script will install the binary in `/usr/local/bin/v2ray` <br>

now create your client configs: <br>
`sudo nano /usr/local/etc/v2ray/config.json` <br>
...
```
in the above config replace "SERVER_IP", "UUID" the port it uses. <br>
<br>
run service commands: <br>
`sudo systemctl daemon-reload` <br>
`sudo systemctl enable v2ray` <br>
`sudo systemctl start v2ray` <br>
<br>
check the connection by curl: <br>
